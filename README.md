# Hi there! :wave: I'm Atmane

## :star2: About Me
- :mortar_board: I'm a student learning to code
- :computer: Currently exploring: cloud
- :video_game: Fun fact: Linus originally wanted to call it freax (free+freak+Unix) J'aime LINUX 
- :round_pushpin: Location: Bejaia

## :rocket: My Coding Journey
I'm just starting my adventure in programming! Here's what I'm excited about:
- Learning Git and GitHub
- Building my first projects
- Connecting with other developers

## :bulb: Current Goals
- [ ] Complete this GitHub challenge
- [ ] Learn DevOps
- [ ] Build my first project
- [ ] Make my first open source contribution

## :mailbox: Let's Connect!
- GitHub: atmcel
- Discord: atmcel

---
:sparkles: *This is my first repository! Watch this space grow!* :sparkles:
# :wave: Welcome to my GitHub

## :smile: Emojis
Use them to make your profile fun! :rocket: :sparkles:

## :clipboard: Lists
### Skills
- :snake: Python
- :globe_with_meridians: HTML / CSS
- :gear: Git & GitHub

### Interests
- :robot: AI
- :bar_chart: Data
- :video_game: Gaming

## :link: Links
- :briefcase: [LinkedIn](https://www.linkedin.com)
- :globe_with_meridians: [Portfolio](https://example.com)
- :bird: [Twitter](https://twitter.com)

## :frame_with_picture: Images
![Profile Picture](https://github.com/USERNAME.png)

## :label: Badges
![GitHub followers](https://img.shields.io/github/followers/USERNAME?style=social)
![GitHub stars](https://img.shields.io/github/stars/USERNAME?style=social)

## :camera_with_flash: Submission Instructions

Once you're done:
1. Copy your repository URL (it looks like: `https://github.com/YOUR-USERNAME/my-first-repo`)
2. Share it in our Discord channel
3. Check out your classmates' repositories and give them a :star: star!


